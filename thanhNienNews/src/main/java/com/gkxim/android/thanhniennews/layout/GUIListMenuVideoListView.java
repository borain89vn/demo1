 ...
 * @author Timon Trinh
 */
public class GUIListMenuVideoListView extends LinearLayout {
	...
	private GUIListMenuVideoAdapter mGuiMenuListAdapter;
	...
	private OnClickListener mOnClickHeaderAndFooter = new OnClickListener() {
		...
		public void onClick(View v) {
			GUIListMenuVideoListView.this.onClick(v);
		}
	};
	 ...
	 * 30-11-2012
	 */
	public GUIListMenuVideoListView(Context context) {
		super(context);
		initListViewItems();
	}
	 ...
	 * 30-11-2012
	 */
	public GUIListMenuVideoListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		initListViewItems();
	}
	...
	private void initListViewItems() {
		...
		inflate(getContext(), R.layout.menu_list_video_layout, this);
		...
		mGuiMenuListAdapter = new GUIListMenuVideoAdapter(getContext());
		if (GUIListMenuVideoAdapter.HAS_ENABLE_WEBEVENT) {
			GKIMLog.l(0, TAG + "=>has enabled Web's Event item");
			mGuiMenuListAdapter.addMenuSection(
					TNPreferenceManager.EXTRAVALUE_ID_WORLDCUP_BANNER,
					"WorldCup 2014", Color.WHITE,
					getResources().getDrawable(R.drawable.ic_launcher), 0);
		}
		...
		if (secIds != null && secIds.length > 0) {
			...
			for (String secId : secIds) {
				...
				mGuiMenuListAdapter.addMenuSection(secId, textTitle,
						colorTextHover,
						TNPreferenceManager.getSecMediaIcon(secId),
						textLeftPadding);
			}
		}
		...
		// Upload imageview.
		mIVUploadVideo = (ImageView) findViewById(R.id.menu_video_upload);
		if (mIVUploadVideo != null) {
			mIVUploadVideo.setOnClickListener(mOnClickHeaderAndFooter);
		}

		TextView textView = (TextView) findViewById(R.id.txt_send_video);
		if (textView != null) {
			textView.setTypeface(TNPreferenceManager.getTNTypeface(),
					Typeface.BOLD);
		}
	}
	...
	public void onClick(View v) {
		...
		case R.id.menu_in_list:
			// Note: this case is handling only for specified EVENT/CAMPAIGN
			// from TNM
			...
			if (v instanceof LinearLayout) {
				...
				GUIListMenuVideoAdapter.ViewHolder menuItem = (com.gkxim.android.thanhniennews.layout.GUIListMenuVideoAdapter.ViewHolder) v
						.getTag();
				if (menuItem != null) {
					if (TNPreferenceManager.EXTRAVALUE_ID_WORLDCUP_BANNER
							.equals(menuItem.id)) {
						...
						GKIMLog.lf(v.getContext(), 1, TAG
								+ "=>Open Worldcup 2014 campaign: " + url);
						if (!TextUtils.isEmpty(url)) {
							try {
								...
								Tracking.sendEvent(
										TNPreferenceManager.EVENT_WORLDCUP_MENU_OPENED,
										null);
								hasProcessed = true;
							} catch (Exception e) {
								GKIMLog.l(
					...
					} else if (TNPreferenceManager.getMediaSectionId()
							.equalsIgnoreCase(menuItem.id)) {
	...
	private boolean showAccountSetting() {
		GKIMLog.log("Nam.Nguyen checkLoggedIn:"
				+ TNPreferenceManager.checkLoggedIn() + " mUserAccount:"
				+ mUserAccount + " iFBLogin:"
				+ TNPreferenceManager.getLoginFBState());
	...
	/**
	 * NEW FEATURE: special Video section Show Video's Home section
	 * 
	 * @param String
	 *            ID of Video section.
	 */
	private void startVideoSectionActivity(String id) {
		...
		// NOTE: "id" should be the same with
		// TNPreferenceManager.getMediaSectionId()
		Context context = this.getContext();
		Intent intent = new Intent(context, VideoSectionActivity.class);
		context.startActivity(intent);
	}

	private boolean startActivityUserNewStory() {
		if (mNewStoryIntent == null) {
			mNewStoryIntent = new Intent(this.getContext(),
					UserNewVideoActivity.class);
	...
	protected void processVideoUpload() {
		if (!TNPreferenceManager.checkLoggedIn()) {
			UIUtils.showToast(getContext(),
					getResources().getString(R.string.request_for_login));
			showLoginDialog();
