...
public class AsyncVideoHomeAdapter extends BaseAdapter {

	private static final String TAG = "AsyncVideoSectionAdapter";
	...
	public class ViewHolder {
		public VideoHomeLayout boxLayout;
...
//        // Spring
	...
	public AsyncVideoHomeAdapter(Context context) {
	...
	private View drawView(int position, View view) {
		...
		if (view == null) {
			...
			view = mInflater.inflate(R.layout.sectionview_video, null);
			holder.boxLayout = (VideoHomeLayout) view.findViewById(R.id.blSection);
		...
		if (mContext instanceof VideoSectionActivity) {
			VideoSectionActivity act = (VideoSectionActivity) mContext;
			holder.boxLayout.setOnClickListener(act.getOnClick());
		}
		holder.sectionId = gp.getSectionId(); // or it always be EXTRAVALUE_SECTION_HOME_VIDEO
		GKIMLog.l(1, TAG + " drawView holder.sectionId:" + holder.sectionId);
		
		if (TNPreferenceManager.getMediaSectionId()
				.equals(holder.sectionId)) {
			holder.title.setVisibility(View.GONE);
		} else {
			holder.title.setVisibility(View.VISIBLE);
			if (TNPreferenceManager.EXTRAVALUE_SECTION_USER_PAGE_VIDEO
					.equals(holder.sectionId)) {
				holder.title
		...
		if (position == 0) {
            ...
            if (TNPreferenceManager.getMediaSectionId()
                ...
                if (GUIListMenuAdapter.HAS_ENABLE_WEBEVENT) {
                    ...
                    holder.webevent.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            ...
                            if (!TextUtils.isEmpty(url) && mContext != null && mContext instanceof SectionActivity) {
                                try{
                                    Intent i = new Intent(mContext, EventFullWebViewWorldCupActivity.class);
                                    i.putExtra(TNPreferenceManager.EXTRAKEY_EVENTWEBVIEW_URL, url);
                                    ((SectionActivity)mContext).startActivity(i);
                                    Tracking.sendEvent(TNPreferenceManager.EVENT_WORLDCUP_HOME_OPENED,null);
                                }catch (Exception e) {
		...
		if (TNPreferenceManager.getXuanSectionId()
				.equals(holder.sectionId) && position == 0) {
			holder.springGreetings.setVisibility(View.VISIBLE);
			holder.tetOfYou.setVisibility(View.VISIBLE);
			holder.springGreetings.setOnClickListener(((SectionActivity) mContext).getOnClick());
			holder.tetOfYou.setOnClickListener(((SectionActivity) mContext).getOnClick());
     ...
     * @param view The View that is needed to apply the margin.
     */
    public void applyFullMargins(View view) {
        ...
        GKIMLog.l(1, TAG + "applyMargins, bannerWidth:" + bannerWidth + ", marginLR:"
                + onecellmargin);
        if (view != null) {
            LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(
                    bannerWidth, LinearLayout.LayoutParams.WRAP_CONTENT);
            layoutParams.setMargins(onecellmargin, onecellmargin, onecellmargin, onecellmargin);
