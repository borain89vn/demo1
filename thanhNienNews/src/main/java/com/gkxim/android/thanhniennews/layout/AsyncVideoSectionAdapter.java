...
public class AsyncVideoSectionAdapter extends BaseAdapter {
	...
	public class ViewHolder {
		public VideoSectionLayout boxLayout;
	...
	public AsyncVideoSectionAdapter(Context context) {
	...
	private View drawView(int position, View view) {
		...
		if (view == null) {
			...
			view = mInflater.inflate(R.layout.sectionview_videosection, null);
			holder.boxLayout = (VideoSectionLayout) view
					.findViewById(R.id.blSection);
		...
		} else if (mContext instanceof VideoSectionActivity) {
		...
		if (mNotification
			...
			holder.boxLayout.refreshBoxStories(holder.boxLayout,
		...
		if (TNPreferenceManager.EXTRAVALUE_SECTION_HOME
			...
			if (TNPreferenceManager.EXTRAVALUE_SECTION_USER_PAGE
				...
				holder.title.setTextColor(TNPreferenceManager
						.getCategoryColorFromPref(gp.getSectionId()));
	...
	public int currentNumPage() {
		int count = 0;
		if (mPagesArray != null && mPagesArray.size() > 0) {
			for (int i = 0; i < mPagesArray.size(); i++) {
				IGenericPage iGenericPage = mPagesArray.get(i);
				if (iGenericPage != null) {
					count += iGenericPage.getBoxStoryCount();
				}
			}
		}
		GKIMLog.l(2, TAG + "==> currentNumPage: " + count);
		return count;
	}

	public boolean isOver() {
		return currentNumPage() > MAX_OVER;
	}
}
