...
public class ViewFlow extends AdapterView<Adapter> {
	...
	private void init() {
		...
		final ViewConfiguration configuration = ViewConfiguration
				.get(getContext());
		if (TNPreferenceManager.hasSectionScroll()) {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		// FIXME: can't handle if ViewFlow requested an EXACTLY mode for
		// width/height onMesure
...
//		GKIMLog.lf(null, 0, mTAG + "=>onInterceptTouchEvent: " + action);
		switch (action) {
	...
	protected void onScrollChanged(int h, int v, int oldh, int oldv) {
		GKIMLog.lf(null, 0, mTAG + "=> scrolling to [" + h + ", " + v + "] from ["
				+ oldh + ", " + oldv + "].");
